...
namespace Callcocam\FlImporter\Actions\Imports;
...
class ImportColumn extends Component
{
    ...
    {
        $guesses = (array)$this->evaluate($this->guesses);
    ...
    {
        $rules = (array) $this->evaluate($this->dataValidationRules);
     ...
     * @return array<mixed>
     */
    public function getNestedRecursiveDataValidationRules(): mixed
    {
    ...
    protected function castStateItem(mixed $state)
    {
        if (is_string($state)) {
            $state = trim($state);
        }
